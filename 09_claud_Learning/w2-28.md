...
**Phase:** 1 | **Week:** 2 | **Day:** 9 of roadmap | **Date:** March 28–29, 2026

> ℹ️ This handoff covers two sessions: Mar 28 (Kali repair) and Mar 29 morning (libcurl GET done, tcpdump still pending). The final complete Mar 29 handoff is in `w1-day9.md`.

### Completed so far
- Fri Mar 20 (Day 1): STL revision — vector, map, unordered_map, set, priority_queue
- Sat Mar 21 (Day 2): Two Sum (LC #1) + Best Time to Buy Stock I (LC #121) — both in C++
- Sun Mar 22 (Day 3): HTTP basics + FastAPI installed + first 3 endpoints + DNS/TCP/IP/ports lesson
- Mon Mar 23 (Day 4): Contains Duplicate (LC #217) + Valid Anagram (LC #242) + C++ key-value store pushed to GitHub
- Tue Mar 24 (Day 5): FastAPI — query params + Pydantic validation + HTTPException (404)
- Wed Mar 25 (Day 6): Group Anagrams (LC #49) ✅ + Top K Frequent Elements (LC #347) ✅
- Thu Mar 26 (Day 7): Linux fundamentals ✅ — all commands covered live in Kali terminal
- Fri Mar 27 (Day 8): Product of Array Except Self (LC #238) ✅ — prefix/suffix O(n), optimised to O(1)
- **Sat Mar 28 (Day 9): Kali Linux environment repair ✅ — GPG key fixed, full-upgrade complete (2972 packages), git/vscode/SSH/repo all working**
- **Sun Mar 29 (partial): libcurl HTTP client GET /users ✅ — C++ program calls FastAPI /users and prints JSON response. Pushed to GitHub.**

### Still pending (Sun Mar 29)
- libcurl POST `/echo` with JSON body (part 2 of libcurl client)
- TCP/IP + DNS + tcpdump live traffic session

### Next session — complete in this order
1. libcurl POST `/echo` with JSON body — push to GitHub
2. TCP/IP + DNS + tcpdump live session
3. Then Mon Mar 30: DSA

### Week 2 remaining tasks

| Date     | Type    | Task |
|----------|---------|------|
| Sun Mar 29 | Project + Linux | libcurl POST /echo + tcpdump (both pending) |
...
## KALI LINUX SETUP — FULLY COMPLETE ✅ (repaired Sat Mar 28)

- Git installed and configured (user.name + user.email set globally)
...
- `sudo apt-get full-upgrade -y` complete — 2972 packages updated
- `libcurl4-openssl-dev` installed ✅ (version 8.19.0)

If full-upgrade needs to be re-run for any reason:
```bash
...
- GET `/items/{item_id}`
- GET `/items`
- POST `/devices` (typo fixed: `temprature` → `temperature` in Device model)
...
## LIBCURL CLIENT — GET /users DONE ✅, POST /echo PENDING ⏳

**Path:** `phase1-backend/week2/libcurl-client/main.cpp`
**Pushed to GitHub:** Yes (GET version) ✅
...
- `curl_easy_init()` — creates the curl handle (the "postman")
- `curl_easy_setopt()` — gives the postman instructions
- `curl_easy_perform()` — sends the request, fetches response
- `curl_easy_cleanup()` — frees memory
- `WriteCallback` — libcurl pushes response in chunks; callback appends each chunk to the string. `nmemb` = number of members (from old C `fread()` convention). `size * nmemb` = total bytes in chunk
...
- `CURLOPT_HTTP_VERSION_1_1` needed — uvicorn dev server only speaks HTTP/1.1, libcurl defaults to HTTP/2

**Next step for libcurl — POST /echo with JSON body:**
- Use `curl_easy_reset()` to wipe settings between requests (mandatory — settings bleed otherwise)
- Set `CURLOPT_POSTFIELDS` with JSON string
- Set `Content-Type: application/json` header via `curl_slist`
- Free header list with `curl_slist_free_all()`
...
**Do NOT attempt raw POSIX sockets.** Attempted and abandoned — too many new concepts at once (struct sockaddr_in, htons, memcpy, recv loops, file descriptors). Come back at Week 6.

---

## TCPDUMP — PENDING ⏳ (Sun Mar 29)

To be done after libcurl POST is complete.

```bash
# DNS live capture
sudo tcpdump -i wlan0 port 53 -n

# HTTP/HTTPS live capture
...
- TCP handshake flags: `[S]` SYN → `[S.]` SYN-ACK → `[.]` ACK
- Layers: IP → TCP → HTTP
...
phase1-backend/
  ...
  week2/
    libcurl-client/main.cpp  ✅ pushed (GET version) — update with POST today
  week3/
    tcp-server/              ← upcoming (NOT raw sockets before Week 6)
```
...
## LINUX COMMANDS — COMPLETED Thu Mar 26 ✅
...
**Kali notes:**
- Use `top` not `htop` (htop install failed — GPG key issue)
- Use `python3 -m uvicorn` not `uvicorn`
- `grep` needs dot: `grep -r "text" .`
- Never use VS Code run button for compiled C++ — always compile manually with `-lcurl`
...
- Kali system was severely outdated — 2972 packages behind, GPG key broken
...
- `nmemb` = number of members — always give full forms, never hide abbreviations
...
**Code understanding rule:** Never write code blindly. Every line must be explained before typing. Never hide abbreviations — always give full forms. If too many new concepts at once — stop and simplify.
...
5. When broken code sent: identify exact issue, explain why wrong, show fixed version
6. For new concepts: explain → show example → connect to something they know → connect to IoT/C++ background
7. IoT background is a strength — connect to MQTT, device data, sensors
...
14. Linux commands must be typed manually — no copy-paste. Motor memory is how they stick
...
**Sun Mar 29 — complete these in order before moving to Mon DSA:**

1. libcurl POST `/echo` with JSON body — extend `main.cpp`, push to GitHub
2. TCP/IP + DNS + tcpdump:
   ```bash
   sudo tcpdump -i wlan0 port 53 -n        # watch DNS in terminal 1
   curl http://example.com                  # trigger it from terminal 2
   sudo tcpdump -i wlan0 port 80 or port 443 -n  # then watch HTTP
   ```
3. Understand the full stack live: DNS resolution → TCP handshake → HTTP request

**After both done — Mon Mar 30:**
- Longest Substring Without Repeat (LC #3) — sliding window, Medium, 45 min
- Sliding Window Maximum (LC #239) — deque-based O(n), Hard, 60 min

**Sharp tip for LC #239:** The deque stores indices, not values. Think of it as a bouncer — anyone smaller than the new arrival who came in before them gets kicked out, because they'll never be the maximum again.
