...
**Phase:** 1 | **Week:** 2 | **Date:** April 1, 2026
...
- Tue Apr 1: FastAPI + SQLite — POST /users + GET /users ✅ — first persistent database, data survives restart
...
These patterns were learned with guided help. Next time a similar problem appears — hints only, no walkthrough. If unsolved in time limit, assign as homework.

| Pattern | Problem Solved With Help | Rule |
|---------|--------------------------|------|
...
Endpoints: GET `/hello`, POST `/echo`, GET `/items/{item_id}`, GET `/items`, POST `/devices`, GET `/users`

### week2/fastapi-sqlite/main.py ✅
Path: `~/Learning-Directory/08_repeat/phase1-backend/week2/fastapi-sqlite/main.py`

Endpoints:
- `POST /users` — inserts name + role into SQLite, returns new user with auto id
- `GET /users` — fetches all users from SQLite, returns as JSON

Key concepts learned:
- `sqlite3.connect()` — opens or creates the .db file
- `cursor.execute()` — runs SQL queries
- `commit()` — saves to disk permanently
- `lastrowid` — gets auto-generated id after INSERT
- `fetchall()` — returns list of tuples from SELECT
- `?` placeholders — never interpolate variables directly into SQL (injection risk)
- `CREATE TABLE IF NOT EXISTS` — safe to run every server start
- `AUTOINCREMENT` — SQLite generates id, never pass manually
...
Key concepts learned:
- `curl_easy_init()` — creates curl handle
- `curl_easy_setopt()` — gives instructions to the handle
- `curl_easy_perform()` — sends the request
- `curl_easy_reset()` — wipes settings before reusing handle
- `curl_easy_cleanup()` — frees memory
- `WriteCallback` — collects chunked response into a string
- Must use `http://` not `https://` for localhost uvicorn
- Must force `CURL_HTTP_VERSION_1_1`
...
phase1-backend/
  ...
  week2/
    ...
    fastapi-sqlite/main.py   ✅ done + pushed
  week3/
    tcp-server/              ← upcoming
```
...
- Active interface: `wlan0` (WiFi)

---

## DEQUE — LEARNED Mon Mar 30 ✅
...
**Critical rule:** Always check `!dq.empty()` before accessing `front()` or `back()`.

```cpp
while(!dq.empty() && nums[dq.back()] < nums[r])  // ✅ empty check FIRST
```
...
**Backend:** Build first, understand after. Every concept must result in running code. FastAPI is the framework — Python syntax is secondary. Not a Python developer — using Python as a tool to build backend systems. Python syntax will come passively through building.
...
- Longest Substring LC #3: O(n) time, O(n) space
- Sliding Window Max LC #239: O(n) time, O(k) space
...
- Kali system was severely outdated — fixed Mar 28
...
- libcurl: VS Code run button doesn't pass `-lcurl`
...
- Deque: used `if` instead of `while` when kicking from back → only kicked one element
- Deque: checked `win.back()` instead of `win.front()` for old index check
- Translating traced examples to code is still a struggle, but improving
- Python indentation — equivalent to `{}` in C++, 4 spaces inside every function/block
- SQLite: `CREATE TABEL` / `EXITS` typos — double check SQL keywords always
- SQLite: `connnect` triple-n typo — always verify function names
- SQLite: `VALUE` instead of `VALUES` in INSERT statement
- Python syntax unfamiliarity — not a concern, Python is a tool not the goal
...
15. Always give full forms of abbreviations — no hiding
16. **Pattern rule:** If a pattern was learned with guided help (see Pattern Tracker), next similar problem = hints only, no walkthrough. If unsolved in time limit, assign as homework.
17. **Claude limit awareness:** Send full lessons in one go — don't send small pieces. Context window is limited, don't waste turns on fragments.
18. **Python rule:** He is not learning Python. Python syntax errors are expected and normal. Focus on system understanding, not syntax correction.

---

## WHERE TO RESUME

**Next session is Wednesday April 2 — DSA day.**

Task: Valid Parentheses (LC #20) + Min Stack (LC #155). Stack problems.

**Homework to check first:** None. All clear.
