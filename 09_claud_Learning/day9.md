...
- Sat Mar 28: Kali Linux environment setup + system repair (in progress — full-upgrade running)

### Current pending tasks
- `sudo apt-get full-upgrade -y` is running on Kali — let it finish completely, do NOT interrupt
- After upgrade finishes: `sudo apt-get install libcurl4-openssl-dev -y`
- Then verify: `curl-config --version`
- Then build the libcurl HTTP client (carried over from Sat Mar 28)
...
## KALI LINUX SETUP — COMPLETED SAT MAR 28

- Git installed and configured
- VS Code installed via Microsoft repo
- SSH key generated and added to GitHub
- Repo cloned: `~/08_repeat/phase1-backend/`
- FastAPI + uvicorn working via `python3 -m uvicorn main:app --reload`
- GPG key fixed: `wget -q -O- https://archive.kali.org/archive-key.asc | gpg --dearmor | sudo tee /etc/apt/trusted.gpg.d/kali-archive-keyring.gpg > /dev/null`
- sources.list: `deb http://kali.download/kali kali-rolling main contrib non-free non-free-firmware`
- `sudo apt-get full-upgrade -y` was running when session ended — must verify it completed next session

### If full-upgrade failed or was interrupted next session:
```bash
...
- GET `/items`
- POST `/devices`
- GET `/users` ← added Sat Mar 28 for libcurl client

Users data hardcoded:
```python
users = [
    {"id": 1, "name": "Ansu", "role": "backend-dev"},
    {"id": 2, "name": "Alice", "role": "iot-engineer"},
]
```

Typo fixed: `temprature` → `temperature` in Device model.

Start server with:
```bash
cd ~/08_repeat/phase1-backend/week1/fastapi-intro
python3 -m uvicorn main:app --reload
```

---

## LIBCURL CLIENT — CARRY OVER TO NEXT SATURDAY

**Goal:** C++ program that calls FastAPI `/users` and prints the response.
**Path:** `phase1-backend/week2/libcurl-client/main.cpp`
**Prerequisite:** `libcurl4-openssl-dev` must be installed first.

The raw sockets approach was attempted but abandoned — too advanced for current stage. Come back to libcurl once system is fixed.

**Important:** Do NOT attempt raw POSIX sockets approach again until at least Week 6. It requires understanding of `struct sockaddr_in`, `htons`, `memcpy`, `recv()` loops, file descriptors — too many new concepts at once for current level.
...
**Code understanding rule:** Never write code blindly. Every line must be explained before typing — what the function does, what the struct is, what the pointer means. If it's too many new concepts at once, stop and simplify.
...
find . -name "name" -type d  → find directory by name

# Permissions
chmod 755 file         → owner=rwx, group=r-x, others=r-x
chmod 644 file         → owner=rw-, group=r--, others=r--
ls -la                 → see permissions column
...
curl -k                      → skip SSL verification

# Terminal copy/paste
Ctrl+Shift+C           → copy in terminal
Ctrl+Shift+V           → paste in terminal
Middle mouse button    → instant paste (Linux way)
```

> **Kali notes:**
> - htop install failed — use `top` instead
> - uvicorn not in PATH — use `python3 -m uvicorn` instead of `uvicorn`
> - grep needs dot: `grep -r "text" .` not just `grep -r "text"`
...
- Kali GPG signing key outdated — fixed Mar 28 (see setup section)
...
- Raw sockets too advanced for current stage — too many new concepts at once (struct sockaddr_in, htons, memcpy, recv loops, file descriptors)
- Kali full-upgrade was needed — system was severely outdated (2972 packages behind)
...
4. When they send broken code: identify exact issue, explain why wrong, show fixed version
5. For new concepts: explain → show example → connect to something they already know → connect to IoT/C++ background
6. IoT background is a strength — connect new concepts to it (MQTT, device data, sensors)
7. Build interactive visual lessons for theory topics
8. When stuck: trace through with actual values step by step
9. After every completed task: tick it off, state what they learned, tell them exactly what's next
...
14. Linux commands must be typed manually every time — no copy-paste. Motor memory is how they stick
...
1. Check if `full-upgrade` completed: `sudo apt-get full-upgrade -y` (run again if it didn't finish)
2. Then: `sudo apt-get install libcurl4-openssl-dev -y`
3. Then: `curl-config --version`
4. If libcurl installs — build the libcurl HTTP client (carried over project)
5. Sunday Mar 29 — complete libcurl client FIRST (install libcurl4-openssl-dev, 
   build main.cpp, compile, run, push to GitHub), THEN do TCP/IP + tcpdump 
   session after. Both must be done in the same session.
